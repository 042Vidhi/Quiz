...
import { quiz5 } from '../../db/big5_ques_db/big5';
import Result from './Result';
import {  Progress } from 'antd';

const Quiz = () => {
  const quizData = quiz5;
  // next set of 6 questions
  ...
  const handleOptionSelect = (option, index,scoretype,type) => {
    const updatedSelectedAnswers = [...selectedAnswers];
    updatedSelectedAnswers[index] = option; // Update selected option for the active question
    setSelectedAnswers(updatedSelectedAnswers);
  };
  

const handleSaveAns = () => {
  // No need to pass index here since we're saving answers for multiple questions
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        {Array.from({ length: 7 }, (_, i) => i + 1).map(option => (
                          <div
                            ...
                            onClick={() => handleOptionSelect(option, index,question.scoretype,question.type)}
                          >
                            <div
